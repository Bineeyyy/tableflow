...
export default function SettingsPage() {
  ...
  const [restaurant, setRestaurant] = useState({
    name: 'Ταβέρνα Αλέξης', address: 'Μητροπόλεως 42, Αθήνα', phone: '210 1234567',
    email: 'info@taverna-alexis.gr', capacity: '13',
  });
  ...
  const [account, setAccount] = useState({
    firstName: 'Αλέξης', lastName: 'Παπαδόπουλος',
    email: 'alexis@taverna-alexis.gr', currentPassword: '', newPassword: '',
  });
          ...
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 space-y-5">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <FieldInput label="Όνομα εστιατορίου" icon={Store} value={restaurant.name}
                      onChange={e => setRestaurant(r => ({ ...r, name: e.target.value }))} />
                  </div>
                  <div className="sm:col-span-2">
                    <FieldInput label="Διεύθυνση" icon={MapPin} value={restaurant.address}
                      onChange={e => setRestaurant(r => ({ ...r, address: e.target.value }))} />
                  </div>
                  <FieldInput label="Τηλέφωνο" icon={Phone} type="tel" value={restaurant.phone}
                    onChange={e => setRestaurant(r => ({ ...r, phone: e.target.value }))} />
                  <FieldInput label="Email επικοινωνίας" icon={Mail} type="email" value={restaurant.email}
                  ...
                  <div>
                    ...
                    <input type="number" min="1" max="200" value={restaurant.capacity}
                      onChange={e => setRestaurant(r => ({ ...r, capacity: e.target.value }))}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-stone-100">
              ...
              <button onClick={handleSave}
                className="flex items-center gap-2 px-6 py-2.5 bg-terracotta hover:bg-terracotta-dark text-white text-sm font-medium rounded-xl transition-colors active:scale-95 shadow-sm shadow-terracotta/20">
                <Save size={15} />
                Αποθήκευση
              </button>
            </div>
